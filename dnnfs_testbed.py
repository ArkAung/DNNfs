...
def findBestHyperparameters(train_images, train_labels, val_images, val_labels):
    ...
    l_rate = [1e-4, 1e-4, 0.001, 0.0015, 0.001, 0.00001, 0.0006, 0.0006, 0.00007, 0.001]
    b_size = [1000, 10000, 50, 125, 500, 275, 88, 40, 625, 25]
    alpha = [0, 0.8, 0.02, 1e2, 5.0, 0.2, 0.3, 0.7, 0.1, 0.9]
    epochs = [10, 20, 50, 10, 30, 40, 30, 20, 10, 40]
    min_cost = 100
    max_acc = 0
    best = 0
    for i in range(10):
...
if __name__ == "__main__":
    ...
    hidden_nodes, learning_rate, batch_size, ridge_term, epochs = 30, 0.001, 50, 0.02, 50
    ...
    dimensions = trainingImages.shape[1]
    classes = trainingLabels.shape[1]

    w1_range = math.sqrt(2. / dimensions)
    w_1 = np.random.uniform(-w1_range, w1_range, (h_nodes, dimensions))
    b_1 = np.ones((1, h_nodes)) * 0.1
    w2_range = math.sqrt(2. / h_nodes)
    w_2 = np.random.uniform(-w2_range, w2_range, (classes, h_nodes))
    b_2 = np.ones((1, classes)) * 0.1
    ...
    print ("Check grad value is ", check_grad(checkgrad_cost, checkgrad_grad, checkgrad_w,
                                              trainingImages[1:10], trainingLabels[1:10]))
